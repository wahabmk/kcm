	...
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			if err != nil && apierrors.IsNotFound(err) {
				sysNS = &corev1.Namespace{Name: testSystemNamespace}
			...
			if err != nil && apierrors.IsNotFound(err) {
				namespace = &corev1.Namespace{Name: nmcsNamespace}
			...
			serviceSetKey = types.NamespacedName{
				Namespace: nmcsNamespace,
				Name:      serviceset.ObjectKey(testSystemNamespace, &clusterDeployment, &kcmv1.NamespacedMultiClusterService{Namespace: nmcsNamespace, Name: nmcsName}).Name,
			}
		...
		It("should ignore selfManagement and never create a management ServiceSet", func() {
			mgmtServiceSetKey := types.NamespacedName{
				Namespace: testSystemNamespace,
				Name:      serviceset.ObjectKey(testSystemNamespace, nil, &kcmv1.NamespacedMultiClusterService{Namespace: nmcsNamespace, Name: nmcsName}).Name,
			}
		...
		It("should not match a ClusterDeployment in a different namespace", func() {
			...
			Eventually(func(g Gomega) {
				...
				otherCDSSKey := client.ObjectKey{
					Namespace: otherNamespace,
					Name:      serviceset.ObjectKey(testSystemNamespace, &otherCD, &kcmv1.NamespacedMultiClusterService{Namespace: nmcsNamespace, Name: nmcsName}).Name,
				}
