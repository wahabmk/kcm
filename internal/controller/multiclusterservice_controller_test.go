...
var _ = Describe("MultiClusterService Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should successfully reconcile the resource", func() {
			...
			By("updating MultiClusterService to set self-management")
			Eventually(func(g Gomega) {
				// Update the MCS
			...
			Eventually(func(g Gomega) {
				...
				// Verify that the ServiceSet for CD no longer exists
				err = k8sClient.Get(ctx, serviceSetKey, &serviceSet)
				g.Expect(apierrors.IsNotFound(err)).To(BeTrue())

				// Verify that the ServiceSet for Management still exists
				g.Expect(k8sClient.Get(ctx, mgmtServiceSetKey, &mgmtServiceSet)).ToNot(HaveOccurred())
			}).Should(Succeed())

			// By("updating MutliClusterService to remove self-management")
			// Eventually(func(g Gomega) {
			// 	// Update the MCS
			// 	g.Expect(k8sClient.Get(ctx, multiClusterServiceRef, multiClusterService)).NotTo(HaveOccurred())
			...
			// 	// Verify that the ServiceSet for Management no longer exists
			...
			Eventually(func(g Gomega) {
				...
				// Verify that the ServiceSet for CD exists
